...
let FAQ = function () {
  ...
  const FAQItem = ({ question, answer }) => {
    ...
    return (
      <div
        className="w-full h-full px-10 py-6 bg-stone-900 justify-start items-center "
      >
          ...
          <div className="flex flex-col items-start gap-6 ">
            <div className="w-full self-stretch h-full py-2.5 border-b border-white flex-col items-start gap-5">
              <div className="w-full py-3.5  border-white flex items-center justify-between  gap-5">
                <div className="text-white w-3/4 text-2xl font-bold font-['Space Grotesk'] leading-7 sm:leading-9">
                  {question}
                </div>
                <div
                  className="text-white text-5xl font-light font-['Inter'] cursor-pointer"
  ...
  const renderQuestions = () => {
    ...
    return questionsToShow.map((item, index) => (
      <FAQItem key={index} question={item.ques} answer={item.ans} />
    ));
  };
  ...
  return (
    <>
      <div id="faq" className="w-full h-full relative">
      ...
      <div className="w-full h-full px-1 sm:px-5 pb-8 bg-stone-900 justify-start items-center ">
        ...
        {!showAll && (
          <div
            className=" w-full px-4 py-2  m-auto  justify-center items-center gap-1 sm:gap-2.5 inline-flex cursor-pointer"
            onClick={handleLoadMore}
          >
            <div className="border-2 ">
              <div className=" m-3 text-center flex justify-center items-center text-white text-2xl font-bold font-['Space Grotesk'] leading-9">
                LOAD MORE +
              </div>
            </div>
          </div>
